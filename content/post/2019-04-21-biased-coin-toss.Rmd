...
res.one %>%
    ggplot(aes(ind, prob)) +
    geom_line() +
    geom_vline(xintercept = min.flip.one$ind,
               lty = 4,
               col = "red") +
    geom_hline(yintercept = min.flip.one$prob,
               lty = 4,
               col = "red") +
    annotate(
        "text",
        x = min.flip.one$ind,
        y = .5,
        label = as.character(min.flip.one$ind)
    ) +
...
res.two %>%
    ggplot(aes(ind, prob)) +
    geom_line() +
    geom_vline(xintercept = min.flip.two$ind,
               lty = 4,
               col = "red") +
    geom_hline(yintercept = min.flip.two$prob,
               lty = 4,
               col = "red") +
    annotate(
        "text",
        x = min.flip.two$ind,
        y = .5,
        label = as.character(min.flip.two$ind)
    ) +
    scale_y_continuous(breaks = seq(.5, 1, .1), labels = scales::percent)
```
